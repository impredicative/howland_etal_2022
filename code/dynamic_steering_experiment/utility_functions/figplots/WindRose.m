...
lablegend        = '$\mathrm{Wind \ Speeds \ in \ m \ s^{-1}}$';
colorfun         = 'jet';
...
%% Title and Legend
title(TitleString,'color',TextColor, 'interpreter', 'latex', 'fontsize', 20);         % Display a title
...
if LegendType==2                                                           % If legend type is box:
    ...
    l       = legend(axs,leyenda,'location','northwest', 'fontsize', 16);                  % Display the legend wherever (position is corrected)
...
function FrequecyLabels(circles,radius,angulo,TextColor)
    ...
    for i=1:length(circles)
        text(radius(i)*c,radius(i)*s,['$',num2str(circles(i),'%.0f') '\%$'],'HorizontalAlignment',ha,'verticalalignment',va,'color',TextColor, 'interpreter', 'latex', 'fontsize', 20); % display the labels for each circle
    end
    ...
    if rmin>0
        ...
        text(rmin*c,rmin*s,'$0\%$','HorizontalAlignment',ha,'verticalalignment',va,'color',TextColor, 'interpreter', 'latex', 'fontsize', 1); % display the labels for each circle
    end
    
function CardinalLabels(circlemax,TextColor,labels)
    text( circlemax-6,-1,[' ' labels.E],'HorizontalAlignment','left'  ,'verticalalignment','middle','color',TextColor, 'interpreter', 'latex', 'fontsize', 30); % East  label
    text(-circlemax+6,1,[labels.W ' '],'HorizontalAlignment','right' ,'verticalalignment','middle','color',TextColor, 'interpreter', 'latex', 'fontsize', 30); % West  label
    text(0, circlemax-6,labels.N      ,'HorizontalAlignment','center','verticalalignment','bottom','color',TextColor, 'interpreter', 'latex', 'fontsize', 30); % North label
    text(0,-circlemax+6,labels.S      ,'HorizontalAlignment','center','verticalalignment','top'   ,'color',TextColor, 'interpreter', 'latex', 'fontsize', 30); % South label
